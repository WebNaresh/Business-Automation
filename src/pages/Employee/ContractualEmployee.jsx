...
const ContractualEmployee = () => {
    ...
    const fetchAvailableEmployee = async (page) => {
        try {
            const apiUrl = `${import.meta.env.VITE_API}/route/employee/get-contract-emloyee/${organisationId}?page=${page}`;
    ...
    const renderPagination = () => {
        ...
        return pageNumbers.map((number, index) => (
            <Button
                ...
                sx={{
                    backgroundColor: number === currentPage ? "#174E63" : "transparent",
                    color: number === currentPage ? "#ffffff" : "#174E63",
                    "&:hover": {
                        backgroundColor: "#133e47",
                        borderColor: "#174E63",
                    },
                    height: "32px",
                    minWidth: "40px",
    ...
    const handleAddEmpOpen = () => setOpen(true);
    const handleAddEmpClose = () => setOpen(false);

    // for update
    const [updateData, setUpdateData] = useState(false);
    const [data, setData] = useState(null);

    const handleUpdateOpen = (items) => {
        setUpdateData(true);
        setData(items);
    };
